...
@Injectable()
export class ParkingConfigHeaderEditGetSecondTimeIsDuplicateMiddleware implements NestMiddleware {
    constructor(
    ...
    async CheckValues(body: any) {
        return await this.checkEditCPHSecondIsDuplicate(body);
    }

    async checkEditCPHSecondIsDuplicate(body: any) {
        ...
        let sql = `select cph_id from m_calculate_parking_header
            where delete_flag = 'N' 
            and cph_priority_no != 1
            and company_id = $1 and cartype_id = $2 
            and cpm_id = $3
            and cph_id != $4
            and ($5 between time_zone_start and time_zone_stop
            or $6 between time_zone_start and time_zone_stop)
            ;`
        const query = {
            text: sql
            , values: [
