...
@Injectable()
export class ParkingConfigMasterMiddleware implements NestMiddleware {
    constructor(
    ...
    async checkMasterByDayTypeMatchCarType(body: any) {
        ...
        switch (day_type) {
            case "N":
                const checkCPMNormalDuplicate = await this.checkMasterIsDuplicateMathCartype(body);
                if (checkCPMNormalDuplicate)
                    return this.errMessageUrilTh.errCPMNormalIsDuplicate;
                return null;
            case "SPECIAL":
                const checkCPMSpecialDuplicate = await this.checkMasterSpecialIsDuplicateMathCartype(body);
                if (checkCPMSpecialDuplicate)
                    return this.errMessageUrilTh.errCPMSpecialDateIsInRage;
                return null;
            case "WEEKEND":
                const checkCPMWeekendDuplicate = await this.checkMasterIsDuplicateMathCartype(body);
                if (checkCPMWeekendDuplicate)
                    return this.errMessageUrilTh.errCPMWeekendIsDuplicate;
                return null;
            case "HOLIDAY":
                const checkCPMHolidayDuplicate = await this.checkMasterIsDuplicateMathCartype(body);
    ...
    async checkMasterSpecialIsDuplicateMathCartype(body: any) {
    ...
    async checkMasterIsDuplicateMathCartype(body: any) {
        const company_id = body.company_id;
        const cartype_id = body.cartype_id;
        const day_type = body.cpm_day_type;
        let sql = `select cpm_id from m_calculate_parking_master
        ...
        const query = {
            text: sql
            , values: [company_id, day_type, cartype_id]
        }
        const res = await this.dbconnection.getPgData(query);
        if (res.error)
            return res.error
