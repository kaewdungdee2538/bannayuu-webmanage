...
@Injectable()
export class GetCartypeNotNullMiddleware implements NestMiddleware {
    constructor(
    ...
    async use(req: Request, res: Response, next: () => void) {
        ...
        if (messageCheckvalue) {
            console.log('Middleware check get cartype not null value  : ' + messageCheckvalue)
            res.send({
                response: {
    ...
    async CheckValues(body: any) {
        if (body.cartype_id) {
            if (this.formatDataUtils.HaveSpecialFormat(body.cartype_id))
