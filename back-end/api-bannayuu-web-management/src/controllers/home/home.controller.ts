...
export class HomeController {
    ...
    async createHome(@Body() body, @Request() req) {
        const checkhomeDuplicate = await this.checkHomeDuplicate(body.home_address, req.user.employee)
        if (checkhomeDuplicate) throw new StatusException({
            error: checkhomeDuplicate,
            result: null,
            message: checkhomeDuplicate,
            statusCode: 200
        }, 200);
        else return this.homeService.createHome(body, req.user);
    }
    //-------------------------------------Edit
    @UseGuards(JwtAuthGuard)
    @Post('get-home-by-id')
    async getHomeInfoById(@Body() body, @Request() req) {
        return this.homeService.getHomeInfoByID(body, req.user);
    }

    @UseGuards(JwtAuthGuard)
    @Post('edit-home')
    async editHomeInfo(@Body() body, @Request() req) {
        const checkhomeInbase = await this.checkHomeInbase(body.home_id, req.user.employee)
        if (checkhomeInbase) throw new StatusException({
            ...
            const checkhomeDuplicate = await this.checkEditHomeDuplicate(body, req.user.employee)
            if (checkhomeDuplicate) throw new StatusException({
                error: checkhomeDuplicate,
                result: null,
                message: checkhomeDuplicate,
    ...
    async checkHomeDuplicate(home_address: any, employee: any) {
        let sql = `select * from m_home where delete_flag = 'N' and company_id = $1 and home_address = $2;`
        const query = {
            text: sql
            , values: [employee.company_id, home_address]
        }
    ...
    async checkEditHomeDuplicate(homeObj: any, employee: any) {
        let sql = `select * from m_home where delete_flag = 'N' and company_id = $1 and home_address = $2 and home_id != $3;`
        const query = {
            text: sql
            , values: [employee.company_id, homeObj.home_address,homeObj.home_id]
        }
        const res = await this.dbconnection.getPgData(query)
        if (res.error)
            return this.errMessageUtilsTh.messageProcessFail
    ...
    async checkHomeInbase(home_id: any, employee: any) {
        let sql = `select * from m_home where delete_flag = 'N' and company_id = $1 and home_id = $2;`
        const query = {
            text: sql
            , values: [employee.company_id, home_id]
        }
        const res = await this.dbconnection.getPgData(query)
        if (res.error)
            return this.errMessageUtilsTh.messageProcessFail
